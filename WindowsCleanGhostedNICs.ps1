...
.SYNOPSIS
    Detects and removes ghosted (disconnected) network interface cards (NICs) from Windows.
    Author: Marcus Ferreira marcus.ferreira[at]microsoft[dot]com
    Version: 0.1    

.DESCRIPTION
    This script scans the Windows registry for network adapters on PCI and VMBUS buses,
    identifies ghosted (disconnected) NICs, and offers to clean them up from the registry.
...
.EXAMPLE
    ...
    PS> .\WindowsCleanGhostedNICs.ps1
#>
...
# Checks for empty root registry keys after ghosted NIC deletion and removes them if found.
...
# Deletes runs pnpclean to clean up device entries and ghosted NICs from the registry.
